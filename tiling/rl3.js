...
function create() {
    ...
    layer1 = map.create('level1', COLS, ROWS, 32, 32);
...
function updateMarker() {
    ...
    {
//        map.putTile(currentTile + 1, currentLayer.getTileX(marker.x), currentLayer.getTileY(marker.y), currentLayer);
...
function drawMap() {
  for (var y = 0; y < ROWS; y++) {
    for (var x = 0; x < COLS; x++) {
      var type = mapState[y][x];
      if(type == '#') {
        map.putTile(1, x, y, currentLayer);
      } else if (type == '.') {
        //map.putTile(1, x, y, currentLayer);
      }
    }  
